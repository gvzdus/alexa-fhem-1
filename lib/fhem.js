...
function
widgetOverride(s) {
  if( !s.Attributes.widgetOverride )
    return;

  for( let na of s.Attributes.widgetOverride.split( ' ' ) ) {
    let parts = na.split( ':', 2 );
    s.PossibleSets = s.PossibleSets.replace(RegExp('\\b'+parts[0]+'(:[^ ]*)?\\b'), na );
  }
}
...
FHEMDevice(platform, s) {
  ...
  if( match = s.PossibleSets.match(/(^| )(rgb)\b/i) )  {
    let setting = match[2];
    if( !this.service_name ) this.service_name = 'light';
    this.mappings.RGB = { reading: setting, cmd: setting };
  }
  ...
  if( match = s.PossibleSets.match(/(^| )ct(:[^\d]*([^\$ ]*))?/) ) {
    ...
    if( match[3] ) {
      ...
      minValue = parseInt(values.slice(-3)[0]);
      maxValue = parseInt(values.slice(-1)[0]);
    }
  ...
  if( s.Readings.mute ) {
    this.mappings.Mute = { reading: 'mute', cmd: 'mute' };

  } else if( s.Readings.Molume ) {
    this.mappings.Mute = { reading: 'Mute', cmd: 'mute' };
  }
  ...
  if( s.Readings.motion ) {
    if( !this.service_name ) this.service_name = 'MotionSensor';
    this.mappings.MotionDetected = { reading: 'motion' };
  } else if( s.Internals.type === 'ZHAPresence' || s.Internals.type === 'ZLLPresence' ) {
    // HUE Motion sensor
    if( !this.service_name ) this.service_name = 'MotionSensor';
    this.mappings.MotionDetected = { reading: 'state' };
  }
  ...
  if( s.Internals.TYPE == 'SONOSPLAYER' ) { //FIXME: use sets [Pp]lay/[Pp]ause/[Ss]top
    ...
    if( !this.service_name ) this.service_name = 'switch';
    this.mappings.On = { reading: 'state', valueOff: '/off|Off|OFF|A0|000000/', cmdOn: 'on', cmdOff: 'off' };
...
//log( util.inspect(s) );
  ...
  if( s.Attributes.alexaProactiveEvents === undefined )
  this.proactiveEvents  = this.connection.fhem.proactiveEvents;
  else
    this.proactiveEvents  = parseInt(s.Attributes.alexaProactiveEvents) == true;
  this.log( s.Internals.NAME+ (this.proactiveEvents ? ' will try to send proactive events' : ' will not send proactive events' ) );
  ...
  this.log.info( s.Internals.NAME + ' uses ID: '+ this.uuid_base );
