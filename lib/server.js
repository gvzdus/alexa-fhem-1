...
Server.prototype.open_ssh = function() {
  if( !ssh_client ) {
    ...
    const args = ['-R', '1234:' + proxy_config['bind-ip']+ ':' + proxy_config.port,
...
function deviceToEndpoints(device) {
  ...
  if( device.isOfType('scene') ) {
    ...
    } else if( device.mappings.On && device.mappings.On.cmdOn ) {
      d.capabilities.push( {
                             "type": "AlexaInterface",
                             "interface": NAMESPACE_SceneController,
                             "version": "3",
                             "supportsDeactivation": (device.mappings.On.cmdOff ? true : false)
                           }
                         );
    }
  ...
  if( device.isOfType('blind') && device.mappings.TargetPosition ) {
    var mapping = device.mappings.TargetPosition;
    d.capabilities.push( {
                           "type": "AlexaInterface",
  ...
  if( device.mappings.Hue  ) {
    d.capabilities.push( {
                           ...
                           "properties": {
                             ...
                             "proactivelyReported": device.proactiveEvents,
                             "retrievable": ((device.mappings.Saturation && device.mappings.Brightness) ? true : false)
                           }
                         }
                       );
  }
  ...
  if( device.mappings.ContactSensorState || device.mappings.CurrentDoorState   ) {
    d.capabilities.push( {
                           "type": "AlexaInterface",
  ...
  if( device.mappings.MotionDetected ) {
    d.capabilities.push( {
                           "type": "AlexaInterface",
  ...
  if( device.mappings.Alarm ) {
    var type = device.mappings.Alarm.type || 'fireAlarm';
    d.capabilities.push( {
                           "type": "AlexaInterface",
  ...
  if( device.mappings.ChannelController ) {
  ...
  if( device.mappings.InputController ) {
    if( d.displayCategories.indexOf('TV') === -1 )
      d.displayCategories.push ( 'TV' );
    let inputs = [];
    if( typeof device.mappings.InputController.value2homekit === 'object' )
      for( var input in device.mappings.InputController.value2homekit )
  ...
  if( device.mappings.PlaybackController ) {
    if( d.displayCategories.indexOf('TV') === -1 )
      d.displayCategories.push ( 'TV' );
    let operations = [];
    if( typeof device.mappings.PlaybackController.value2homekit === 'object' )
      for( var operation in device.mappings.PlaybackController.value2homekit )
  ...
  if( device.mappings[FHEM.CustomUUIDs.Volume] || device.mappings.Mute ) {
    ...
    if( device.mappings.Mute ) capability.properties.supported.push( { "name": "mute" } );
    if( device.mappings[FHEM.CustomUUIDs.Volume] ) capability.properties.supported.push( { "name": "volume" } );
    d.capabilities.push( capability );
  }
...
var handleColorController = function(event) {
  ...
  switch (requestedName) {
    case 'SetColor':
      ...
      // Ignore brightness on color change, see https://developer.amazon.com/de/docs/device-apis/alexa-colorcontroller.html
      if ( device.mappings.Brightness ) {
        let mapping = device.mappings.Brightness;
        let current_brightness = device.fhem.reading2homekit(mapping, device.fhem.cached(mapping.informId));
        if (current_brightness) {
          target_brightness = current_brightness;
          log.info("Maintaining current brightness " + current_brightness);
        }
      }
      break;
    default:
